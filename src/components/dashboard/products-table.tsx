...
export function ProductsTable({ products, allCategories }: ProductsTableProps) {
  ...
  // Fermer menu si clic en dehors d'un menu ouvert
  // On utilise un click handler sur le document mais seulement
  // si le menu est ouvert, évitant les conflits avec les boutons du menu.
  React.useEffect(() => {
    if (!openMenuId) return;
    function handleOutside(e: MouseEvent) {
      const target = e.target as HTMLElement;
      // Ne pas fermer si le clic est dans un menu dropdown
      if (target.closest("[data-menu-dropdown]")) return;
      setOpenMenuId(null);
    }
    // Utiliser setTimeout pour laisser le click courant se propager d'abord
            ...
            return (
              ...
              >
                ...
                <div className="w-11 h-11 rounded-lg bg-zinc-100 overflow-hidden shrink-0 relative">
                  {thumb ? (
                    <Image src={thumb} alt={product.name} fill className="object-cover" sizes="44px" />
                  ) : (
                ...
                <div className="relative flex justify-end">
                  ...
                  {/* Dropdown — data-menu-dropdown pour exclure du click outside */}
