...
export function ProductForm({ categories, productId, defaultValues }: ProductFormProps) {
  ...
  async function onSubmit(data: FormData) {
    ...
    // Composer l'origine depuis pays + ville
    const countryName = COUNTRIES.find((c) => c.code === data.originCountry)?.name ?? "";
    const origin = data.originCity && countryName
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-8">
      ...
      <section className="bg-white rounded-xl border border-zinc-100 p-6 space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <Input {...register("name")} placeholder="Ex: Boubou Brodé Prestige" maxLength={100} />
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-xs text-zinc-400">{watch("description")?.length ?? 0}/1000</span>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          {errors.priceCfa && <p className="text-xs text-red-600">{errors.priceCfa.message}</p>}
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Combobox
                ...
                onChange={(v) => setValue("originCountry", v, { shouldValidate: true })}
            ...
            <div className="space-y-1.5">
              ...
              <Combobox
                options={cityOptions}
                value={selectedCity}
                onChange={(v) => setValue("originCity", v, { shouldValidate: true })}
                placeholder={selectedCountry ? "Sélectionnez une ville" : "Choisissez un pays d'abord"}
      ...
      <section className="bg-white rounded-xl border border-zinc-100 p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className={cn(
            ...
            selectedCategoryIds.length === 3 ? "text-amber-600" : "text-zinc-400"
          )}>
      ...
      <section className="bg-white rounded-xl border border-zinc-100 p-6 space-y-4">
        ...
        <p className="text-xs text-zinc-400">
          Ajoutez jusqu&apos;à 2 photos. La première sera utilisée comme photo principale.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <ImageUploader
            ...
            required
            value={imageMain ?? ""}
            onChange={(url) => setValue("imageMain", url, { shouldValidate: true })}
          />
          <ImageUploader
            slot="second"
            ...
            required={false}
            value={imageSecond ?? ""}
            onChange={(url) => setValue("imageSecond", url, { shouldValidate: true })}
          />
        </div>
        {errors.imageMain && (
...
const schema = z.object({
  ...
  imageMain: z.string().min(1, "L'image principale est obligatoire"),
  imageSecond: z.string().optional(),
  // boolean strict (pas de .default() pour éviter boolean | undefined dans le type inféré)
  inStock: z.boolean(),
});
