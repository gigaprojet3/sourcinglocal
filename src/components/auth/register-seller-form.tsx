...
import { z } from "zod";
import { Eye, EyeOff, Loader2, User, Mail, Lock, Phone, Store } from "lucide-react";
...
export function RegisterSellerForm() {
  ...
  async function onSubmit(data: FormData) {
    ...
    // Connexion auto puis onboarding boutique
    ...
    if (signInResult?.ok) {
      ...
      router.push("/connexion?registered=1&role=seller");
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
      ...
      {/* Info */}
      <div className="flex items-start gap-3 rounded-lg bg-zinc-50 border border-zinc-100 p-3">
        <Store size={15} strokeWidth={1.5} className="text-zinc-500 shrink-0 mt-0.5" />
        <p className="text-xs text-zinc-500 leading-relaxed">
          Après votre inscription, vous configurerez votre boutique pour commencer à vendre.
        </p>
      </div>
      ...
      <Field label="Nom complet" error={errors.name?.message}>
        <div className="relative">
          ...
          <Input {...register("name")} placeholder="Jean Dupont" className="pl-9" autoComplete="name" />
        </div>
      </Field>

      <Field label="Adresse email professionnelle" error={errors.email?.message}>
        <div className="relative">
          <Mail size={15} strokeWidth={1.5} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400" />
          <Input {...register("email")} type="email" placeholder="vous@boutique.com" className="pl-9" autoComplete="email" />
        </div>
      </Field>
      ...
      <Field label="Numéro de téléphone" error={errors.phone?.message}>
        <div className="relative">
          ...
          <Input {...register("phone")} type="tel" placeholder="+242 06 000 0000" className="pl-9" autoComplete="tel" />
        </div>
      </Field>

      <Field label="Pays d'activité" error={errors.country?.message}>
        <Combobox
      ...
      <Field label="Mot de passe" error={errors.password?.message}>
        ...
        <p className="text-xs text-zinc-400 mt-1">Min. 8 caractères, 1 majuscule et 1 chiffre.</p>
      </Field>
      ...
      <Button type="submit" className="w-full mt-2" disabled={isSubmitting}>
        {isSubmitting ? (
          <><Loader2 size={15} strokeWidth={1.5} className="animate-spin" /> Création en cours...</>
        ) : (
